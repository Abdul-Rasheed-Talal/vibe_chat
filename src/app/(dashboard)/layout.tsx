...
export default function DashboardLayout({
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-background text-foreground">
            ...
            <aside className="hidden md:flex w-20 lg:w-80 border-r border-border/40 bg-card/50 backdrop-blur-xl flex-col z-20 transition-all duration-300">
                <SidebarContent pathname={pathname} theme={theme} setTheme={setTheme} />
            </aside>

            {/* Main Content (Center Column) */}
            <main className="flex-1 relative overflow-hidden flex flex-col pt-16 md:pt-0 pb-16 md:pb-0">
                ...
                <div className="flex-1 z-10 overflow-hidden grid grid-cols-1 xl:grid-cols-[1fr_auto] gap-0">
                    <div className="flex flex-col overflow-hidden h-full">
                        {children}
                    </div>
                    {showRightPanel && (
                        <aside className="hidden xl:flex w-80 border-l border-border/40 bg-card/30 backdrop-blur-xl flex-col p-6 z-20 h-full overflow-y-auto">
...
function SidebarContent({ pathname, theme, setTheme }: { pathname: string, theme: string | undefined, setTheme: (theme: string) => void }) {
    return (
        <div className="flex flex-col h-full py-6">
            <div className="mb-8 px-6 flex items-center gap-2 lg:px-6 justify-center lg:justify-start">
